...
onMounted(async () => {
  ...
  // 等两帧确保 splash 已完成绘制，再显示窗口
  await new Promise<void>(resolve => {
    requestAnimationFrame(() => requestAnimationFrame(() => resolve()))
  })
  try { getCurrentWindow().emit('frontend-ready') } catch {}
...
:root {
  ...
  /* Radius scale — aligned with cc-switch */
  --radius-sm: 6px;
  --radius-md: 8px;
  --radius-lg: 12px;
...
/* Scrollbar — hidden by default (cc-switch style) */
::-webkit-scrollbar {
  width: 4px;
  height: 4px;
}
...
/* App entrance animation (cc-switch fade-in) */
