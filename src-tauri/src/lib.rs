...
use std::sync::Arc;
use tauri::{
    AppHandle, Emitter as TauriEmitterTrait, Listener, Manager, WebviewWindow,
    menu::{Menu, MenuItem},
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            if let Ok(resource_dir) = app.path().resource_dir() {
                if let Some(dir_str) = resource_dir.to_str() {
                    let current_path = std::env::var("PATH").unwrap_or_default();
                    let new_path = format!("{};{}", dir_str, current_path);
                    std::env::set_var("PATH", &new_path);
                    info!("已将资源目录加入 PATH: {:?}", resource_dir);
                }
            }
            ...
            // 延迟显示窗口：等待前端内容加载完成后再显示，消除 WebView2 初始化白闪
